...
void BTInsert ( tBTNodePtr * RootPtr, int Content )
{
  ...
  else
  {
    ...
    tBTNodePtr previousNode = (* RootPtr);

		while ( currentNode != NULL )
		{
			previousNode = currentNode;

			if ( Content > currentNode->Cont ) currentNode = currentNode->RPtr;
			else if ( Content < currentNode->Cont ) currentNode = currentNode->LPtr;
      else
			{
				currentNode = NULL;
				previousNode = NULL;
      ...
      tBTNodePtr newNode = ( tBTNodePtr ) malloc ( sizeof ( struct tBTNode ) );
        ...
        exit( 1 );
			...
			if ( Content > previousNode->Cont ) previousNode->RPtr = newNode;
			else if ( Content < previousNode->Cont ) previousNode->LPtr = newNode;
    }
  }
}
...
void BTPreorder ( tBTNodePtr RootPtr )
{
  if ( RootPtr != NULL )
	{
  	tStackP * pointerStack = ( tStackP * ) malloc ( sizeof ( tStackP ) );

    if ( pointerStack == NULL )
    {
      fprintf( stderr, "Error: Unable to allocate required memory.\nDetails: Failed to create a new stack.\nIn: void BTPreorder ( tBTNodePtr )" );
      exit( 1 );
    }

  	SInitP( pointerStack );
  	Leftmost_Preorder( RootPtr, pointerStack );

  	while ( !SEmptyP( pointerStack ) )
  	{
  		RootPtr = STopPopP( pointerStack );
  		Leftmost_Preorder( RootPtr->RPtr, pointerStack );
  	}

    free( pointerStack );
  }
  else fprintf( stderr, "Error: Unable to perform requested action.\nDetails: Procedure called on an empty tree ( tBTNodePtr is NULL ).\nIn: void BTPreorder ( tBTNodePtr )" );
}
...
void BTInorder ( tBTNodePtr RootPtr )
{
  if ( RootPtr != NULL )
    ...
    if ( pointerStack == NULL )
    {
      fprintf( stderr, "Error: Unable to allocate required memory.\nDetails: Failed to create a new stack.\nIn: void BTInorder ( tBTNodePtr )" );
      exit( 1 );
    }

  	SInitP( pointerStack );
  	Leftmost_Inorder( RootPtr, pointerStack );

  	while ( !SEmptyP( pointerStack ) || RootPtr->RPtr != NULL )
  	{
      RootPtr = STopPopP( pointerStack );
  		BTWorkOut( RootPtr );
  		...
  		{
  			Leftmost_Inorder( RootPtr->RPtr, pointerStack );
  		}
  	}
...
void BTPostorder ( tBTNodePtr RootPtr )
{
  if( RootPtr != NULL )
    ...
    if ( pointerStack == NULL )
    {
      fprintf( stderr, "Error: Unable to allocate required memory.\nDetails: Failed to create a new pointer stack.\nIn: void BTPostorder ( tBTNodePtr )" );
      exit( 1 );
    }

    SInitP( pointerStack );

  	tStackB * visitedStack = ( tStackB * ) malloc ( sizeof ( tStackB ) );

    if ( visitedStack == NULL )
    {
      fprintf( stderr, "Error: Unable to allocate required memory.\nDetails: Failed to create a new bool stack.\nIn: void BTPostorder ( tBTNodePtr )" );
      free( pointerStack );
      exit( 1 );
    }
  	...
  	Leftmost_Postorder( RootPtr, pointerStack, visitedStack);

  	while( !SEmptyP( pointerStack ) )
  	{
  		RootPtr = STopPopP( pointerStack );

  		if( STopPopB( visitedStack ) )
  		{
  			SPushP( pointerStack, RootPtr );
  			SPushB( visitedStack, false );
  			Leftmost_Postorder( RootPtr->RPtr, pointerStack, visitedStack );
  		}
...
void BTDisposeTree ( tBTNodePtr * RootPtr )
{
  if ( (* RootPtr) != NULL )
    ...
    if ( pointerStack == NULL )
    {
      fprintf( stderr, "Error: Unable to allocate required memory.\nDetails: Failed to create a new stack.\nIn: void void BTPreorder ( tBTNodePtr )" );
      exit( 1 );
    }
  ...
  else fprintf( stderr, "Error: Unable to perform requested action.\nDetails: Procedure called on an empty tree ( tBTNodePtr is NULL ).\nIn: void BTDisposeTree ( tBTNodePtr * )" );
}
